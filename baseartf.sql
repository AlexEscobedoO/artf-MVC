...
CREATE DATABASE IF NOT EXISTS baseartf;

#Seleccionamos la DB
USE baseartf;

#Creamos la tabla sin relación

CREATE TABLE IF NOT EXISTS users(
iduser int auto_increment,
tipouser enum ('admin','second') not null,
nomuser varchar (25) not null,
pauser varchar (25) not null,
sauser varchar (25),
...
idempre int auto_increment,
rsempre varchar (250) not null,
dirempre varchar (350) not null,
tipoempre enum ('asignatario','particular','concesionario','permisionario') not null,
primary key (idempre)
)engine=InnoDB;

CREATE TABLE IF NOT EXISTS fabricante(
idfab int auto_increment,
rsfab varchar (100) not null,
marfab varchar (100) not null,
primary key (idfab)
)engine=InnoDB;

CREATE TABLE IF NOT EXISTS modelo(
idmod int auto_increment,
modequi varchar (50) not null,
primary key (idmod)
)engine=InnoDB;

#Creamos las tablas que si tienen relacion

create table if not exists solrf(
idsol int auto_increment,
idempsol int,
idusersol int,
numacuofsol varchar (30) not null,
obssol varchar (200),
fecapsol date,
docsol longblob,
primary key (idsol),
foreign key (idempsol) references empresa (idempre),
foreign key (idusersol) references users (iduser)
)engine=InnoDB;

create table if not exists insrf(
idins int auto_increment,
idempins int,
idsolins int,
iduserins int,
numacuofins varchar (30)not null,
obsins varchar (200),
fecapins date,
docins longblob,
primary key (idins),
foreign key (idempins) references empresa (idempre),
foreign key (idsolins) references solrf (idsol),
foreign key (iduserins) references users (iduser)
)engine=InnoDB;


CREATE TABLE IF NOT EXISTS rectrf(
    
idrect int auto_increment,
idinsrect int,
iduserrect int,
numacuofrect varchar (30) not null ,
fichatecrect longblob,
numdocemp varchar (30) not null ,
fechadocsol date not null,
fecharect date not null,
desrect text ,
obsrect text,
acurect blob ,
claverect varchar (100) not null ,
fechamodrect date not null,
primary key (idrect),
foreign key (idinsrect) references insrf(idins),
foreign key (iduserrect) references users(iduser)
)engine=InnoDB;

CREATE TABLE IF NOT EXISTS modrf(
    
idmod int auto_increment,
idrectmod int,
idusermod int,
numacuofmod varchar (30) not null ,
acumod longblob,
fechamod date not null ,
desmod text not null,
obsmod text not null,
fichatecmod longblob not null ,
clavemod varchar (100) not null,
primary key (idmod),
foreign key (idrectmod) references rectrf(idrect),
foreign key (idusermod) references users(iduser)
)engine=InnoDB;


CREATE TABLE IF NOT EXISTS canrf(
    
idcan int auto_increment,
idmodcan int,
idusercan int,
numacuofcan varchar (30) not null,
fechaofcan date not null,
juscan text not null,
obscan text,
fichacan longblob not null,
clavecan varchar (100) not null,
fechacan date not null,
primary key (idcan),
foreign key (idmodcan) references modrf(idmod),
foreign key (idusercan) references users(iduser)
)engine=InnoDB;

create table if not exists equiuni(
idequi int auto_increment,
idempreequi int,
idfabequi int,
idmodeequi int,
idsolequi int,
...
idmodequi int,
idcanequi int,
modaequi enum('Arrastre','Tractivo','Trabajo') not null,
tipequi enum('Coche','Coche domo/comedor','Coche terraza','Coche bar','Furgon','Gondola','Plataforma','S169','Tolva','Locomotora','Locomotora-AC','Locomotora-DC','Locomotora-EVO'),
combuequi enum('Diesel','Gasolina','Electrico'),
pequi int,
nserie varchar (50),
regiequi enum('Arrendado','Propio'),
graequi enum('Si','No'),
usoequi enum('Carga','Pasajeros','Patio','Mixto'),
fcons year,
nofact varchar (50),
tcontra varchar (50),
fcontra date,
...
fecharequi date,
primary key (idequi),
foreign key (idempreequi) references empresa(idempre),
foreign key (idfabequi) references fabricante(idfab),
foreign key (idmodeequi) references modelo(idmod),
foreign key (idsolequi) references solrf(idsol),
foreign key (idinsequi) references insrf(idins),
foreign key (idrectequi) references rectrf(idrect),
foreign key (idmodequi) references modrf(idmod),
foreign key (idcanequi) references canrf(idcan)

)engine=InnoDB;


#Creamos las tablas del modulo de permisos 

create table if not exists permisoins(
    IDPERINS INT PRIMARY KEY AUTO_INCREMENT,
    idpermiempre int,
    NUMPER VARCHAR(50),
    NUMOFPER VARCHAR(50),
    FECHAPER DATE,
    OBPER LONGTEXT,
    TIPOPER ENUM('TERMINALES DE PASAJEROS', 'TERMINAL DE CARGA ESPECIALIZADA', 'TERMINAL DE CARGA ORDINARIA', 'TERMINAL INTERIOR DE CARGA', 'TRANSBORDO Y TRASVASE DE LÍQUIDOS', 'TALLERES DE MANTENIMIENTO Y EQUIPO FERROVIARIO', 'CENTROS DE ABASTO PARA LA OPERACIÓN DE EQUIPOS', 'PERMISOS PARA REALIZAR MANIOBRAS EN ZONAS FEDERALES TERRESTRES PÚBLICOS', 'PERMISOS PARA REALIZAR MANIOBRAS EN ZONAS FEDERALES TERRESTRES PRIVADOS'),
    LUGAPER VARCHAR(800),
    ESTAPER ENUM('Estado1', 'Estado2'),
    MUNPER ENUM('Municipio1', 'Municipio2'),
    OBRPER LONGTEXT,
    VIGENPER VARCHAR(100),
    CONDPER LONGTEXT,
    NOAUTPRO VARCHAR(250),
    AREARES VARCHAR(250),
    AREAPRO VARCHAR(150),
    FECHAAPPRO DATE,
    PROEJEPER TEXT,
...
CREATE TABLE if not exists permisomod (
    ...
    OBSPER LONGTEXT,
    RFMOBS LONGTEXT,
    TIPOASIENTOMOD VARCHAR(250),
    NUMMOD INT,
    DESMOD VARCHAR(250),
    PDFMOD BLOB,
    CONSMOD LONGTEXT,
    NUMOFIMOD VARCHAR(50),
    FECHAOFIMOD DATE,
    IDEMPRESA INT,
    DOMICILIO INT,
    HOMOINS VARCHAR(150),
...
CREATE TABLE if not exists PERMRECT (
    ...
    INSTNOT VARCHAR(800),
    OBSPER LONGTEXT,
    RFM_OBSERVACIONES LONGTEXT,
    TIPO_ASIENTO_RECT VARCHAR(250),
    NUMERO_RECTIFICACION INT,
    DESCRIPCION_RECTIFICACION VARCHAR(250),
    PDF_RECTIFICACION BLOB,
    CONSIDERACIONES_RECTIFICACION LONGTEXT,
    NUMERO_OFICIO_RECTIFICACION VARCHAR(50),
    FECHA_OFICIO_RECTIFICACION DATE,
    IDEMPRESA INT,
    DOMICILIO INT,
...
CREATE TABLE if not exists permican (
    ...
    OBSPER LONGTEXT,
    RFM_OBSERVACIONES LONGTEXT,
    TIPO_ASIENTO_CAN VARCHAR(250),
    ULTIMO_ASIENTO_REGISTRADO VARCHAR(50),
    DESCRIPCION_CANCELACION VARCHAR(250),
    PDF_CANCELACION BLOB,
    JUSTIFICACION_CANCELACION LONGTEXT,
    NUMERO_OFICIO_CANCELACION VARCHAR(50),
    FECHA_OFICIO_CANCELACION DATE,
    FOREIGN KEY (idpermiempre) REFERENCES empresa(idempre)
...
-- Constraints for table `estados_municipios`
--
ALTER TABLE `estados_municipios`
  ADD CONSTRAINT `municipios_id_refs_id_6d8b23ec` FOREIGN KEY (`municipios_id`) REFERENCES `municipios` (`id`),
  ADD CONSTRAINT `estados_id_refs_id_cb2ad4e5` FOREIGN KEY (`estados_id`) REFERENCES `estados` (`id`);
